...
setTimeout(function() {
    ...
    // adding markers to the layer recreationcenters
    for (let i = 0; i < recreationcenters.length; i++) {
        marker = L.marker([recreationcenters[i][0], recreationcenters[i][1]], {
            icon: iconRecreation
        });
        pB.addLayer(marker).bindPopup(recreationcenters[i][2]);
    }

    // adding markers to the layer schools
    for (let i = 0; i < schools.length; i++) {
        marker = L.marker([schools[i][0], schools[i][1]], {
            icon: iconSchool
        });
        pC.addLayer(marker).bindPopup(schools[i][2]);
    }
}, 3000);
